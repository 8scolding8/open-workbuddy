"""FastAPI proxy bridging OpenAI-compatible clients to the WorkBuddy API."""
...
app = FastAPI(title="Open WorkBuddy Proxy", version="1.2.0", lifespan=lifespan)
...
def normalize_model(model_name: str | None) -> str:
    ...
    for available in AVAILABLE_MODELS:
        if cleaned == str(available.get("id") or "").lower():
            return str(available["id"])
    return model_name
...
def workbuddy_transport(
    base_url: str | None = None,
    cwd: str | None = None,
    model: str | None = None,
) -> WorkBuddyAcpTransport:
    transport = WorkBuddyAcpTransport.from_config(config, base_url=base_url, model=model)
    if cwd:
        transport.cwd = cwd
    return transport
...
async def stream_workbuddy_chat(
    ...
    *,
    ...
    for attempt in range(attempts):
        ...
        stream = None
        try:
            transport = workbuddy_transport(candidate, cwd=project, model=model)
...
async def complete_workbuddy_chat(
    ...
    *,
    ...
    parts = []
    async for event in stream_workbuddy_chat(
        messages,
...
async def proxy_config(request: Request):
    ...
    if denied:
        return denied
    config.refresh_available_models()
    base_url = f"{str(request.base_url).rstrip('/')}/v1"
    return {
        "base_url": base_url,
        "model": sidecar_manager.model,
...
@app.post("/proxy/model")
async def select_proxy_model(request: Request):
    denied = require_local_request(request)
    if denied:
        return denied
    config.refresh_available_models()
    ...
    selected = next(
        (
            ...
            if str(item.get("id") or "").lower() == requested.lower()
        ),
        None,
    )
    if not selected:
        return error_json(404, f"Unknown WorkBuddy model: {requested or '<empty>'}", "not_found_error")
    try:
        changed = await sidecar_manager.set_model(selected)
        config.save_value("WORKBUDDY_MODEL", selected)
    except (OSError, SidecarError) as exc:
        return error_json(500, str(exc), "configuration_error")
    return {
        "status": "ok",
        "model": sidecar_manager.model,
        "changed": changed,
        "persisted": True,
        "sidecars_recycled": changed,
    }
...
if __name__ == "__main__":
    ...
    if config.TRANSPORT == "http" and not config.DEFAULT_API_KEY and sys.stdin.isatty():
        try:
            key_input = getpass.getpass("Enter your WorkBuddy API Key: ")
            if key_input.strip():
