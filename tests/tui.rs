...
fn launcher_reuses_a_current_release_binary_without_cargo() {
    ...
    let binary = root.path().join("target/release/open-workbuddy");
    std::fs::write(
        &binary,
        "#!/bin/bash\nprintf '%s\\n' \"$*\" > \"$BINARY_LOG\"\n",
    )
...
fn shortcuts_distinguish_send_newline_cancel_and_quit() {
    assert!(matches!(
        ...
        Some(open_workbuddy::tui::app::Action::Submit)
    ));
    assert!(matches!(
        event::key_action(KeyEvent::new(KeyCode::Enter, KeyModifiers::ALT), false, 80),
        Some(open_workbuddy::tui::app::Action::Newline)
    ));
    assert!(matches!(
        event::key_action(
        ...
        Some(open_workbuddy::tui::app::Action::Cancel)
    ));
    assert!(matches!(
        event::key_action(
        ...
        Some(open_workbuddy::tui::app::Action::QuitRequested)
    ));
}

#[test]
fn composer_editing_shortcuts_map_to_selection_actions() {
    use open_workbuddy::tui::app::Action;
    for (key, expected) in [
        ('a', Action::SelectAll),
...
fn all_documented_shortcuts_have_semantic_actions() {
    use open_workbuddy::tui::app::{Action, Modal};
    let cases = [
        (
