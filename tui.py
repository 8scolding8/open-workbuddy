#!/usr/bin/env python3
"""Interactive Terminal User Interface (TUI) & Tester for Open WorkBuddy Proxy."""
...
def render_banner():
    ...
    grid.add_row(
        "[bold magenta]⚡ OPEN WORKBUDDY PROXY TUI & TESTER[/bold magenta]",
...
def set_api_key_prompt():
    render_banner()
    console.print(Panel("[bold yellow]Set / Change WorkBuddy API Key[/bold yellow]", border_style="yellow"))
    current = get_current_key()
    if current:
        console.print(f"Current Key: [dim]{current[:4]}...{current[-4:] if len(current)>8 else ''}[/dim]")

    new_key = Prompt.ask("[bold green]Enter your WorkBuddy API Key[/bold green]", password=True)
    if new_key.strip():
...
def main_menu():
    while True:
        ...
        table.add_row("[2]", "🔑 Set / Update WorkBuddy API Key")
