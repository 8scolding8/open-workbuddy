...
class SidecarManager:
    def __init__(
        ...
        self.model = str(model).strip() or "hy3"
    ...
    def _process_matches(pid: int, marker: str) -> bool:
        if os.name == "nt":
            # Windows has no /proc. In-process sidecars are tracked directly;
            # persisted foreign PIDs are not trusted or terminated here.
            return False
        try:
    ...
    def _launch_command(self, arguments: list[str]) -> list[str]:
        # WorkBuddy packages `codebuddy` as a JavaScript launcher without a
        # Windows executable extension. Invoke it through Node.js on Windows;
        # native launchers remain unchanged.
        if os.name == "nt" and self.cli_path.suffix.lower() not in {".exe", ".com", ".cmd", ".bat"}:
            node = os.environ.get("CODEBUDDY_NODE_BIN") or shutil.which("node") or shutil.which("node.exe")
            if not node:
                raise SidecarError(
                    "Node.js was not found for the WorkBuddy JavaScript CLI. "
                    "Install Node.js or set CODEBUDDY_NODE_BIN."
                )
    ...
    async def ensure(self, session: dict) -> str:
        ...
        async with self._lock(session_id):
            ...
            if managed is not None and managed.poll() is None and url and await self._healthy(url):
            ...
            try:
                ...
                process = subprocess.Popen(self._launch_command(arguments), **popen_kwargs)
            finally:
                log_handle.close()
    ...
    async def set_model(self, model: str) -> bool:
        """Switch the default model and recycle sidecars before the next request."""
        selected = str(model).strip()
        if not selected:
            raise SidecarError("WorkBuddy model must not be empty")
