...
async fn health(State(s): State<AppState>) -> Json<Value> {
    ...
    Json(
        json!({"status":"ok","service":"open-workbuddy","version":env!("CARGO_PKG_VERSION"),"build_id":crate::BUILD_ID,"uptime_seconds":s.started_at.elapsed().as_secs(),"logical_service":s.config.base_url,"transport":s.config.transport,"upstream_mode":upstream_mode}),
    )
}
