...
fn validate_health(health: &Health) -> Result<(), String> {
    if health.service != "open-workbuddy" || health.status != "ok" {
        return Err("The configured port is not a compatible WorkBuddy proxy".into());
    }
    if health.build_id != crate::BUILD_ID {
        return Err(format!(
            "The configured port is running a different WorkBuddy proxy build (expected {}, received {})",
            crate::BUILD_ID,
...
mod tests {
    ...
    fn rejects_a_different_server_build() {
        ...
        assert!(error.contains("different WorkBuddy proxy build"));
