...
#[command(
    name = "open-workbuddy",
    version,
    about = "OpenAI-compatible WorkBuddy proxy using official WorkBuddy ACP"
)]
struct Cli {
    #[command(subcommand)]
...
fn ensure_tui_api_key(
    ...
    println!(
        "No usable WorkBuddy API key was found. Enter it once to save it privately in config.json."
    );
    let value = read_key()?;
...
async fn main() -> Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "open_workbuddy=info,tower_http=info".into()),
        )
        .init();
    let cli = Cli::parse();
    ...
    if matches!(command, Command::Tui) {
        config = ensure_tui_api_key(&root, config, || {
            open_workbuddy::tui::setup::read_secret("WorkBuddy API key")
        })?;
    }
    match command {
        Command::Server { .. } => serve(AppState::new(config)?).await?,
        Command::Tui => open_workbuddy::tui::run(&config).await?,
        Command::Key {
            command: KeyCommand::Set { value },
        } => {
            let value = match value {
                Some(value) => value,
                None => open_workbuddy::tui::setup::read_secret("WorkBuddy API key")?,
            };
            if value.trim().is_empty() {
                anyhow::bail!("API key was empty; configuration was not changed");
            }
...
mod tests {
    ...
    use open_workbuddy::config::Config;
