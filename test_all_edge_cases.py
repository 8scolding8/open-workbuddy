"""Exhaustive Edge-Case & Integration Test Suite for Open WorkBuddy Proxy."""
...
def run_exhaustive_tests():
    print("==========================================================")
    print("      EXHAUSTIVE TEST SUITE - OPEN WORKBUDDY PROXY      ")
    print("==========================================================")
    ...
    with httpx.Client(timeout=20.0) as client:
        ...
        # Test 3: POST /v1/chat/completions (model="hy3")
        try:
            p = {"model": "hy3", "messages": [{"role": "user", "content": "Hello"}]}
            r = client.post(f"{BASE_URL}/v1/chat/completions", json=p, headers=headers)
            assert r.status_code == 200
            assert "choices" in r.json()
            print("[PASS 3/15] POST /v1/chat/completions (hy3)")
            passed += 1
        except Exception as e:
            print("[FAIL 3/15] POST /v1/chat/completions (hy3):", e)
            failed += 1

        # Test 4: POST /v1/chat/completions (model="hy3" with space)
        try:
            p = {"model": "hy3", "messages": [{"role": "user", "content": "Test space model"}]}
            r = client.post(f"{BASE_URL}/v1/chat/completions", json=p, headers=headers)
            assert r.status_code == 200
            assert "choices" in r.json()
        ...
        # Test 6: POST /v1/chat/completions (model="workbuddy-default")
        try:
            p = {"model": "workbuddy-default", "messages": [{"role": "user", "content": "OpenCode test"}]}
            r = client.post(f"{BASE_URL}/v1/chat/completions", json=p, headers=headers)
            assert r.status_code == 200
            print("[PASS 6/15] POST /v1/chat/completions (workbuddy-default alias)")
            passed += 1
        except Exception as e:
            print("[FAIL 6/15] POST /v1/chat/completions (workbuddy-default alias):", e)
            failed += 1
        ...
        try:
            p = {"model": "hy3", "messages": [{"role": "user", "content": "No auth header"}]}
        ...
        try:
            p = {"model": "hy3", "messages": [{"role": "user", "content": "Dummy token"}]}
